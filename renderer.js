...
class Renderer extends cutil.mixin(Obj, pubsub, iwx) {
  ...
  getComponent(node) {
    let { x } = this;
    return x.kind(node) === "text"
      ? TextComponent
      : x.kind(node) === "comment"
        ? CommentComponent
        : x.kind(node) === "element"
          ? this.translator &&
            x.ns(node) in this.translator &&
            x.nm(node) in this.translator[x.ns(node)]
    ...
    function dir(folder, prefix) {
      for (let dirent of fs.readdirSync(folder, { withFileTypes: true })) {
        ...
        if (dirent.isDirectory()) {
          ...
          let template = x.root(
            x.fromStr(new Textual({ fn, cs }).read().string),
          );
          ((renderer.translator ||= {})[ns] ||= {})[tag] = class extends (
            TemplateComponent
          ) {
            static {
